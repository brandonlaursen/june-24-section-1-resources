...
try {
  ...
  runSQL = (statement, dbTestFile) => {
    ...
    //!!START SILENT
    if (!dbTestFile) throw new Error('dbTestFile required: No database file specified in runSQL.');
    //!!END
  ...
  runSQLQuery = (statement, dbTestFile) => {
    ...
    //!!START SILENT
    if (!dbTestFile) throw new Error('dbTestFile required: No database file specified in runSQLQuery.');
    //!!END
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        const db = sqlite3(dbTestFile);
...
const runMigrations = async function (dbTestFile) {
  //!!START SILENT
  if (!dbTestFile) console.error('WARNING while migrating: No database file specified. Using process.env.DB_TEST_FILE instead.');
  //!!END
  return new Promise((resolve, reject) => {
    const migrate = exec(
...
const removeTestDB = async function (dbTestFile) {
  //!!START SILENT
  if (!dbTestFile) console.error('WARNING while deleting db: No database file specified. Using process.env.DB_TEST_FILE instead.');
  //!!END
  return new Promise((resolve, reject) => {
    const deleteDB = exec(
...
const seedAllDB = async function (dbTestFile) {
  //!!START SILENT
  if (!dbTestFile) console.error('WARNING while seeding: No database file specified. Using process.env.DB_TEST_FILE instead.');
  //!!END
  return new Promise((resolve, reject) => {
...
module.exports.seedDBFile = async function (fileName, dbTestFile) {
  //!!START SILENT
  if (!dbTestFile) console.error('WARNING while seeding file: No database file specified. Using process.env.DB_TEST_FILE instead.');
  //!!END
  return new Promise((resolve, reject) => {
    const seedProcess = exec(
